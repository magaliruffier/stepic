...
###
      ...
      my $readthrough = qq(<span class="glossary_mouseover">readthrough<span class="floating_popup">$glossary{'readthrough'}</span></span>);
      if ($genome_container->get_coding_count() ){
        if ($genome_container->get_rcoding_count() ){
          $gene_stats{'coding'} = $genome_container->get_coding_count() . " (incl. " . $genome_container->get_rcoding_count() . " $readthrough)";
        } else {
          $gene_stats{'coding'} = $genome_container->get_coding_count();
        }
      }
      ...
      if ($genome_container->get_alt_coding_count() ){
        if ($genome_container->get_alt_rcoding_count() ){
          $alt_gene_stats{'alt_coding'} = $genome_container->get_alt_coding_count() . " (incl. " . $genome_container->get_alt_rcoding_count() . " $readthrough)";
        } else {
          $alt_gene_stats{'alt_coding'} = $genome_container->get_alt_coding_count();
        }
      }
      ...
      if ($genome_container->get_snoncoding_count() ){
        if ($genome_container->get_rsnoncoding_count() ){
          $gene_stats{'shortnoncoding'} = $genome_container->get_snoncoding_count() . " (incl. " . $genome_container->get_rsnoncoding_count() . " $readthrough)";
        } else {
          $gene_stats{'shortnoncoding'} = $genome_container->get_snoncoding_count();
        }
      }
      $title{'shortnoncoding'} = $genome_container->get_attrib('snoncoding_cnt')->name() if $genome_container->get_snoncoding_count();
      print STDERR "Short non coding:$gene_stats{'shortnoncoding'}\n" if $DEBUG;

      if ($genome_container->get_lnoncoding_count() ){
        if ($genome_container->get_rlnoncoding_count() ){
          $gene_stats{'longnoncoding'} = $genome_container->get_lnoncoding_count() . " (incl. " . $genome_container->get_rlnoncoding_count() . " $readthrough)";
        } else {
          $gene_stats{'longnoncoding'} = $genome_container->get_lnoncoding_count();
        }
      }
      $title{'longnoncoding'} = $genome_container->get_attrib('lnoncoding_cnt')->name() if $genome_container->get_lnoncoding_count();
      print STDERR "Long non coding:$gene_stats{'longnoncoding'}\n" if $DEBUG;

      if ($genome_container->get_alt_snoncoding_count() ){
        if ($genome_container->get_alt_rsnoncoding_count() ){
          $alt_gene_stats{'alt_shortnoncoding'} = $genome_container->get_alt_snoncoding_count() . " (incl. " . $genome_container->get_alt_rsnoncoding_count() . " $readthrough)";
        } else {
          $alt_gene_stats{'alt_shortnoncoding'} = $genome_container->get_alt_snoncoding_count();
        }
      }
      $title{'alt_shortnoncoding'} = $genome_container->get_attrib('snoncoding_acnt')->name() if $genome_container->get_alt_snoncoding_count();
      print STDERR "Alternate short non coding:$alt_gene_stats{'alt_shortnoncoding'}\n" if $DEBUG;

      if ($genome_container->get_alt_lnoncoding_count() ){
        if ($genome_container->get_alt_rlnoncoding_count() ){
          $alt_gene_stats{'alt_longnoncoding'} = $genome_container->get_alt_lnoncoding_count() . " (incl. " . $genome_container->get_alt_rlnoncoding_count() . " $readthrough)";
        } else {
          $alt_gene_stats{'alt_longnoncoding'} = $genome_container->get_alt_lnoncoding_count();
        }
      }
      $title{'alt_longnoncoding'} = $genome_container->get_attrib('lnoncoding_acnt')->name() if $genome_container->get_alt_lnoncoding_count();
      print STDERR "Alternate long non coding:$alt_gene_stats{'alt_longnoncoding'}\n" if $DEBUG;

      if ($genome_container->get_pseudogene_count() ){
        if ($genome_container->get_rpseudogene_count() ){
          $gene_stats{'pseudogene'} = $genome_container->get_pseudogene_count() . " (incl. " . $genome_container->get_rpseudogene_count() . " $readthrough)";
        } else {
          $gene_stats{'pseudogene'} = $genome_container->get_pseudogene_count();
        }
      }
      $title{'pseudogene'} = $genome_container->get_attrib('pseudogene_cnt')->name();
      print STDERR "Pseudogenes:$gene_stats{'pseudogene'}\n" if $DEBUG;

      if ($genome_container->get_alt_pseudogene_count() ){
        if ($genome_container->get_alt_rpseudogene_count() ){
          $alt_gene_stats{'alt_pseudogene'} = $genome_container->get_alt_pseudogene_count() . " (incl. " . $genome_container->get_alt_rpseudogene_count() . " $readthrough)";
        } else {
          $alt_gene_stats{'alt_pseudogene'} = $genome_container->get_alt_pseudogene_count();
        }
      }
